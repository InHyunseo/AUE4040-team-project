...
rosbag에서 stride 간격으로 프레임을 뽑아 라벨링 툴(CVAT)에 올릴 jpg로 저장한다.
...
출력:
  <out>/lane/<bag>_<idx:06d>.jpg   → CVAT polygon 세그 (좌실선/우실선/중앙점선)
  <out>/front/<bag>_<idx:06d>.jpg  → CVAT bbox (car)
"""
...
def main():
    ...
    if do_lane:
        print(f"  Lane  → CVAT polygon seg (좌실선/우실선/중앙점선)")
    if do_front:
        print(f"  Front → CVAT bbox (car)")
