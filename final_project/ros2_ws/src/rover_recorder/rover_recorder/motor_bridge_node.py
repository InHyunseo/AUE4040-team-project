...
sys.path.insert(0, "/home/hyunseo/Personal_Research/AUE4040/main")
try:
...
def mix(linear_x: float, angular_z: float, max_speed: float) -> tuple[float, float]:
    ...
    base = abs(linear_x)
    L = max(-max_speed, min(max_speed, base * max(0.0, 1.0 - turn)))
    R = max(-max_speed, min(max_speed, base * max(0.0, 1.0 + turn)))
    if linear_x > 0:    # positive linear_x = reverse on this wiring (rare in teleop)
        L, R = -L, -R
    return L, R
