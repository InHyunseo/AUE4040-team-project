...
cd ~/team/final_project/ros2_ws
source install/setup.bash
ros2 launch rover_lane drive.launch.py monitor:=false     # 최종 주행: 모니터 끄고 자원 절약
...
2. **[로버] bag → labels 일괄 추출** — `--bag_root` 에 bag 들이 든 폴더(record.launch
   ...
   `<out_dir>/<bag폴더명>.h5` 로 만든다(모델 1회만 로드). 기존 17개 + 새 코너 bag 이
   ...
   python3 data_pipeline/extract_labels.py --bag_root rover_data \
       ...
       --out_dir labels_all
   # → labels_all/<bag폴더명>.h5 들이 생성됨
   ```

   > bag 하나만 추출하려면 `--bag rover_data/<name> --out labels_all/<name>.h5`.
...
- **제어율이 낮으면** SegFormer가 병목 — TensorRT로 변환하면 효과 큼. (overlay_viz의 3Hz는
  디버그용 인위적 캡이지 SegFormer 성능 한계가 아니다.)
...
- [ ] (제어율 낮고 주행 끊길 때만) SegFormer TensorRT 변환 → [CHECKLIST](CHECKLIST.md) 참고
...
- **주행 의도 모니터링 오버레이:** 추론 노드가 예측 `waypoints`를 lane 위에 그려
  (`viz.draw_intent`) `/lane_intent/compressed`로 publish → :8080 실시간 확인. 현재
  `drive.launch.py`의 monitor는 raw lane/front만 보여준다. 필요 시 overlay_viz 패턴을 따라 추가.
