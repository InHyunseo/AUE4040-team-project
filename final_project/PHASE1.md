...
> CVAT task **2개**를 따로 만든다: Lane = polygon segmentation,
> Front = bbox(rectangle). 같은 task에 섞지 말 것.
...
### Lane jpg → polygon 세그
- 클래스 3개: **left-solid / right-solid / center-dashed** (polygon)
  (라벨 이름은 노트북 `CVAT_LABEL2ID`와 맞추면 됨)
- 100~300장
- Export: **Segmentation mask 1.1** (`SegmentationClass/` PNG + `labelmap.txt`)
...
1. CVAT에서 polygon 세그 → **Segmentation mask 1.1** export → zip 다운로드
   - 클래스 순서 고정: 배경 0 / 좌실선 1 / 우실선 2 / 중앙점선 3
     (extract_labels.py `SegFormerLaneSeg`의 id2label과 일치해야 함)
   - 노트북 `CVAT_LABEL2ID`를 CVAT 라벨 이름에 맞춤 (색→id 변환은 labelmap.txt로 자동)
