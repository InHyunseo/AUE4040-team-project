...
```
ControlHead   → steer, throttle      (Tanh [-1,1])
WaypointHead  → waypoints (5점, 2.5s) (미터)
```

> **확정 주행 경로 = image→waypoint→pure pursuit→steer.** 멀티태스크로 둘 다 학습하지만,
> 검증·확정된 추론 조향은 **waypoint 출력에 pure pursuit**를 건 것이다(`steer_source:=waypoint`
> 기본값). ControlHead 의 raw steer 는 학습 정확도 하한(tanh 포화 + teleop raw 라벨 weaving)에
> 걸려, 같은 backbone 에서 더 부드러운 waypoint 회귀를 추종하는 쪽이 안정적이었다. raw steer 를
> 직접 쓰려면 `steer_source:=head`(폴백, 비추천).

### waypoint → steer (확정 경로)

추론 노드(`rover_lane`)가 waypoint 를 pure pursuit 로 조향 의도(steer ∈ [-1,1])로 바꾼 뒤,
teleop 과 동일한 제어 계약(`rover_common.control.steer_to_cmd_vel`)으로 cmd_vel 역변환:

```
linear.x  = -(0.20 + abs(steer) * 0.05)   # -0.20 ~ -0.25  (음수 = 전진)
angular.z = steer * 1.2                     # -1.2 ~ +1.2     (양수 = 우회전)
```

- waypoint GT는 cmd_vel을 forward-Euler로 적분한 미래 **2.5초** 궤적(로봇 프레임 5점, x=전방/y=좌).
- pure pursuit 부호: wp.y<0(우)이 우회전이고 angular.z>0이 우회전이라 `steer = -gain·곡률`
  (e2e_infer_node.waypoint_steer). steer_mode(pursuit/heading/max_y/mean)로 회피 강도 튜닝.
- horizon이 짧으면 저속 주행에서 점이 뭉쳐 trivial → 궤적 정보가 약함.
...
- throttle 출력은 cmd_vel에 안 씀 — 속도는 `|steer|`에 커플링(teleop과 동일).

loss: `1.0 * steer + 0.5 * throttle + 0.5 * waypoint` (모두 MSE). steer/throttle 헤드는
backbone 을 제어 신호 쪽으로도 붙잡아 두는 보조 역할로 함께 학습된다(추론 조향은 waypoint 가 담당).
...
(`BASE_V=0.20, TURN_V=0.25, MAX_OMEGA=1.2, TURN_FRAC=(0, 0.8, 1.0)` — `rover_common.constants`)
...
/front_image ─→ resize224 → YOLO → composite_front ────────────────┤→ e2e.engine
                                                                    │   → steer, throttle, waypoints
       steer = waypoint pursuit(default) 또는 ControlHead head  ←──────┘   (throttle은 진단용)
                          linear.x = -(0.20+|steer|*0.05)
...
- **최종 주행 권장값.** 디버그 모니터와 오버레이 JPEG 비용을 줄이려면
...
5. H5 audit + E2E 학습 (Kaggle/Colab) → ONNX export
     legacy H5: WP_FIX_SIGN=True / 새 H5: False
     ResNet18×2 + ControlHead + WaypointHead  [training/e2e-train_kaggle.ipynb → e2e.onnx]
