# AUE4040 — Autonomous Rover

closed-world 회전교차로 자율주행 로버. NVIDIA Jetson Orin Nano + ROS 2 Humble + TensorRT 10 기반.

좌/우 두 미션 중 하나가 런타임에 latch되고, 로버는 E2E 주행 모델로 차선을 추적하면서 표지판·신호·차량·사람은 YOLO + 손코딩 FSM으로 처리한다.

---

## 1. 시스템 구성

closed-world 회전교차로 트랙에서 로버가 수행하는 4가지:

1. **차선 추적** — ResNet18 기반 E2E 주행 모델 3개 (`common` / `left` / `right`)이 rectified 이미지에서 (steer, speed)를 직접 출력
...
- **미션 latch + grace window**: 표지판은 1회만 latch되고, 출발 후 grace 시간 경과 시점에 자동 분기 (분기 시점에 표지판이 다시 보일 필요 없음)
- **세그먼트별 분리 학습**: `common` / `left` / `right` 모델을 별도 세션으로 학습 → 유사 경로 데이터가 누적되어 서로 다른 task 간 액션이 평균화되는 현상 방지
...
│   ├── inference/              로컬 추론 입력
│   ├── inference_out/          로컬 추론 결과
│   └── labeling/               데이터셋 라벨링 보조 스크립트
├── calibration/                IMX219 듀얼 CSI 캘리브레이션 (jetcam)
├── control/                    UART 베이스 컨트롤러 참조 구현
...
Jetson 자체 카메라 드라이버와 주행 제어 명령 동기화를 위해 Python 환경을 사용. `record_and_label` ipynb 하나로 데이터 수집과 주행 명령을 동시에 수행 — 사용자가 키보드로 주행하면 그 시점의 이미지·텔레옵 키·세그먼트 라벨이 한 줄로 기록되어 GT 데이터 수집이 단순해진다.

```
annotation.txt 한 줄:
  {name}.jpg segment steer_tel speed_tel last_key step
```
...
- **step embedding**: 1-D scalar → 32-D MLP (단순 concat하면 FC가 무시함)
- **출력**: 6-class action logits (up / down / left / right / straight / space)
- **postprocess**: argmax → `ACTIONS[idx]`로 (steer, speed) 룩업

회귀가 아닌 action classification을 쓰는 이유: 사람 텔레옵 키 입력이 이미 6개 이산 액션이고, 회귀로 풀면 평균값 액션이 자주 나와 약하게 휘기(jerky and short turns)가 됨. 상위 주행 명령(키보드 버튼) 자체를 라벨로 사용해 학습 난이도를 낮춤.

step 스칼라를 입력에 함께 넣어 회전구간 trajectory의 어디쯤인지 모델에 알려준다 — 같은 이미지라도 시작/중간/끝에서 다른 액션을 낼 수 있도록.

### 3.3 학습 흐름

```
teleop 주행 (record_and_label.ipynb) → annotation.txt
    │
    ▼
preprocess.py
  ...
  · down/space (텔레옵 보정 키) 기본 제외
    │
    ▼
Colab에서 git clone 후 train_e2e.py
  · segment별로 ActionCNN 학습 (별도 모델 3개)
  · class-balanced sampling, weighted CE loss
  · metadata.json에 step_max 기록
    │
    ▼
export_to_onnx.py → (Jetson) trtexec → e2e_{common,left,right}.engine
```
...
Robust한 데이터 확보를 위해 rover 카메라 촬영 이미지와 핸드폰 촬영 이미지를 모두 수집. Roboflow의 augmentation을 활용해 데이터를 증강하고, GPU 가속을 위해 Colab에서 드라이브 연동 후 fine-tune.
...
`imgsz=320`을 선택한 이유: Orin Nano 추론 예산 (stock 640 대비 ~4배 빠름). 표지판·차량 모두 충분히 큰 객체라 작은 객체 detection은 불필요.
...
ultralytics가 아닌 TensorRT를 직접 호출하는 이유: ultralytics import 오버헤드 회피, pycuda 의존성 제거, Orin Nano 30Hz 예산. letterbox + numpy NMS는 [`yolo_inference.py`](main/ros2_ws/src/rover_perception/rover_perception/yolo_inference.py)에 직접 구현.

### 4.5 left/right 동시 감지 처리

YOLO NMS는 클래스 간 중복을 거르지 않아서 같은 표지판에 left/right 박스가 함께 잡힘. `decision_node`에서 프레임당 score 큰 쪽만 채택해 stabilizer 카운터에 반영 → 안정화 임계 도달 시 score가 우세했던 쪽으로 mission latch.
...
여러 노드와 두 모델 실행을 용이하게 하기 위해 ROS 2 사용. 카메라는 기존 Jetson 드라이버로 띄운 스트림을 ROS 2 토픽으로 변환해 publish하고, 주행에 필요한 모든 노드를 동시에 순차 기동하는 launch 파일을 구성.
...
(YOLO TRT)         (E2E TRT, model_manager)
   ↓                    ↓
/detections         /bc_cmd
   └─────────┬──────────┘
             ▼
...
| `rover_lane` | E2E TRT 추론 + 모델 스왑 | pub `/bc_cmd` |
...
Stabilizer는 클래스별 연속 감지 카운터를 유지하다가 한 프레임이라도 사라지면 0으로 리셋, 임계 N프레임 도달 시 `stable=True`. 단일 프레임 오인식 무시.

### 5.5 미션 분기 흐름

```
1. 출발 → COMMON, common 모델로 차선 추적
2. left/right 표지판 5프레임 연속 안정 감지
   → self.mission = "left" or "right" latch
   → 이후 turn-sign 감지는 무시 (한 번만 결정)
3. 출발 후 common_grace_s 경과
   → 자동 STOPPED(entered_by="turn") 진입, 2초 정지
4. stop 타이머 만료 → TURNING 전이
   → active_model() = mission 반환
   → /active_model 발행 → lane_node가 모델 스왑 (step 카운터 리셋)
5. mission 모델로 회전 구간 주행
6. lane_lost → ARRIVED (최종 정지)
```

분기 시점에 표지판이 다시 보일 필요 없음 — `self.mission`이 latch 시점에 기억된다. grace window가 출발 직전 표지판을 잘못 trigger하는 것을 방지. `allowed_missions=["right"]` 설정으로 left 모델이 없을 때 stray 감지로 인한 오latch 차단.

### 5.6 안전 종료

`control_node.main()`에서 `try / except KeyboardInterrupt / finally: node.stop_motors()` 패턴 사용. Ctrl+C 종료 시 마지막 cmd_vel이 남아 로버가 계속 전진하려 하는 문제를 사전 차단 — 종료 직전 명시적으로 throttle 0을 UART로 송신.
...
├── e2e_common.engine         공통 구간
├── e2e_common.metadata.json  step_max 등
├── e2e_right.engine          우회전 구간 (또는 e2e_left.engine)
...
[`main/ros2_ws/src/rover_stereo/config/stereo_calib.yaml`](main/ros2_ws/src/rover_stereo/config/stereo_calib.yaml)은 카메라 마운트 확정 직후 한 번 생성한 뒤 학기 동안 재생성 금지. 이 파일이 바뀌면 그 이전 주행 학습 데이터가 무효화된다. 카메라가 흔들리면 재캘리브가 아니라 마운트 물리 복원으로 대응.
